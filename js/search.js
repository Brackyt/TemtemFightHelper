//Thanks https://codepen.io/davilious/pen/YqdoyP, modified to be adapted
(function() {
    ...
    var SearchOnList = {
        ...
        _handleQueryChanged: function(e, data) {
            this.itemsMatched = this.items.map(function(item) {
                if (format(item.name.toLowerCase()).match(format(data.query.toLowerCase()))) {
                    return {
                        name: item.name,
                        visible: true,
        ...
        _handleResults: function() {
            this.$list.empty();
            this._renderItemsVisible().forEach((item, i) => {
